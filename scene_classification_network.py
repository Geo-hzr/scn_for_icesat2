...
def modified_point_net(input_data, normal_vector, channels):
    ...
    x = Dense(3 * 3, weights=[np.zeros([2 * channels, 3 * 3]), np.eye(3).flatten().astype(np.float32)])(x)
    ...
    f = Dense((1 * channels) * (1 * channels), weights=[np.zeros([(2 * channels), (1 * channels) * (1 * channels)]), np.eye((1 * channels)).flatten().astype(np.float32)])(f)
